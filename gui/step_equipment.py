...
class EquipmentStep(WizardStep):
    ...
    def build_ui(self):
        ...
        ttk.Label(self.frame, text="Choose equipment from your class and background options.",
        ...
        self.summary_text = tk.Text(self.summary_frame, wrap=tk.WORD, height=8,
    ...
    def _populate_class_equipment(self):
        ...
        if not cls:
            ttk.Label(self.class_equip_frame, text="No class selected",
                      style="Dim.TLabel").pack(padx=8, pady=4)
            return

        equip_options = cls.get("starting_equipment", [])
        if not equip_options:
            ttk.Label(self.class_equip_frame, text="No equipment options available",
                      style="Dim.TLabel").pack(padx=8, pady=4)
            return
    ...
    def _populate_bg_equipment(self):
        ...
        if not bg:
            ttk.Label(self.bg_equip_frame, text="No background selected",
                      style="Dim.TLabel").pack(padx=8, pady=4)
            return

        equip_options = bg.get("equipment", [])
        if not equip_options:
            ttk.Label(self.bg_equip_frame, text="No equipment options available",
                      style="Dim.TLabel").pack(padx=8, pady=4)
            return
